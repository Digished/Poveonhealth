...
export function Navbar() {
  ...
  // Check if a doctor session is active (lightweight check)
  useEffect(() => {
    ...
    <nav className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-white/60 shadow-sm">
      <div className="max-w-2xl mx-auto px-4 py-2.5 flex items-center justify-between gap-3">
        ...
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <PoveonLogo className="w-7 h-7" />
          <span className="text-sm font-bold text-slate-800 hidden sm:block">Poveon</span>
        </Link>
          ...
          {!isLoading && loggedIn && typeof session === "object" && session !== null && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 hover:text-red-600 px-2.5 py-1.5 rounded-lg hover:bg-red-50 transition"
                title="Sign out"
              >
          ...
          {!isLoading && !loggedIn && (
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-1.5 text-slate-500 hover:text-slate-700 text-xs font-medium px-2.5 py-1.5 rounded-lg hover:bg-slate-100 transition"
              >
              ...
              {loginOpen && (
                <div className="absolute right-0 top-full mt-2 w-60 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden z-50 animate-fade-in-up">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Manage test requests</p>
                    </div>
                  </Link>
                  ...
                  <Link
                    ...
                    className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition group"
                  >
                    <div className="w-8 h-8 rounded-xl bg-medical-50 border border-medical-100 flex items-center justify-center shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Track referrals & results</p>
                    </div>
                  </Link>
                  ...
                  <Link
                    href="/login"
                    onClick={() => setLoginOpen(false)}
                    className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition group border-t border-slate-100"
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">View your test results</p>
                    </div>
                  </Link>

                  <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50/50">
                    <p className="text-xs text-slate-400 leading-relaxed">New here? No need to sign up — just submit a request to get started.</p>
                  </div>
                </div>
              )}
            </div>
