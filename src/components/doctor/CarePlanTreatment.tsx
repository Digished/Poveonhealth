...
export function CarePlanTreatment({
  ...
  return (
    <CollapsibleSection
      ...
      icon={<ClipboardList className="h-4 w-4 shrink-0 text-medical-500" />}
      count={plan?.items.length}
      // A draft waiting on the doctor is marked rather than forced open: a
      // section that cannot be folded while a suggestion sits in it is a
      // section that cannot be folded.
      alert={!!suggested}
      action={
        canEdit ? (
          <button
