...
import { Mail, KeyRound, ArrowRight, RefreshCw, ChevronLeft, HeartPulse, CreditCard, Lock, ShieldCheck } from "lucide-react";
import { PoveonLogo } from "@/components/PoveonLogo";
import { useRouter } from "next/navigation";

type Stage = "credentials" | "pin" | "otp" | "create-pin";
...
function HmoLoginInner() {
  ...
  async function handleCredentialsSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/hmo/check-pin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email.trim(), policy_number: policyNumber.trim() }),
      });
  ...
  async function handlePinSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/hmo/verify-pin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email.trim(), policy_number: policyNumber.trim(), pin: code }),
      });
      const data = await res.json();
  ...
  async function handleCreatePin(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/hmo/set-pin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-3xl shadow-xl p-6">
          ...
          {stage === "pin" && (
            <form onSubmit={handlePinSubmit} className="space-y-4">
              <button type="button" onClick={() => { setStage("credentials"); setPin(["", "", "", ""]); setError(""); }}
          ...
          {stage === "otp" && (
            <form onSubmit={handleOtpSubmit} className="space-y-4">
              <button type="button" onClick={() => { setStage("credentials"); setOtp(["", "", "", "", "", ""]); setError(""); setAfterOtp("dashboard"); }}
                className="flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600 transition mb-1">
                <ChevronLeft className="w-3 h-3" /> Change details
              </button>
              <div>
          ...
          {stage === "create-pin" && (
            <form onSubmit={handleCreatePin} className="space-y-5">
              ...
              <div className="text-center">
                <button type="button" onClick={() => router.replace("/hmo/dashboard")}
