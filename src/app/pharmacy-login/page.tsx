...
/** Partner pharmacies sign in with a code emailed to their registered address. */
          ...
          {stage === "email" ? (
            <>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Enter your registered pharmacy email and we&apos;ll send a 6-digit code.
              </p>
              <input
                autoFocus
                type="email"
                ...
                className="mt-5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/40"
              />
            ...
            <>
              ...
              <input
                ...
                placeholder="000000"
                className="mt-5 w-full rounded-xl border border-slate-200 px-4 py-3 text-center font-mono text-2xl font-bold tracking-[0.4em] text-slate-800 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/40"
              />
              {error && <p className="mt-3 text-sm text-red-600">{error}</p>}
              <button
                onClick={verify}
              ...
              <button onClick={sendCode} disabled={busy} className="mt-2 w-full py-2 text-xs font-semibold text-slate-400 transition hover:text-slate-600">
                Send it again
              </button>
            </>
          )}
