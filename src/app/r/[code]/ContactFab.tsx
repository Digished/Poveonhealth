...
export default function ContactFab({ phones, whatsapps }: ContactFabProps) {
  const [open, setOpen] = useState(false);

  const totalContacts = phones.length + whatsapps.length;
  if (totalContacts === 0) return null;

  // Single WhatsApp, no phones — link directly
  if (whatsapps.length === 1 && phones.length === 0) {
    return (
  ...
  // Single phone, no WhatsApp — link directly
  if (phones.length === 1 && whatsapps.length === 0) {
    return (
      <div className="fixed bottom-6 right-4 z-50">
        <a
          href={`tel:${phones[0]}`}
  ...
  // Multiple contacts — FAB opens a bottom-sheet modal
  return (
    <>
      {/* Backdrop */}
      {open && (
      ...
      {/* Bottom sheet */}
      {open && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-3xl shadow-2xl px-5 pt-4 pb-8 safe-area-inset-bottom">
          {/* Handle */}
          ...
          <div className="space-y-3">
            ...
            {whatsapps.map((wa, i) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-4 p-4 rounded-2xl border border-green-100 bg-green-50 hover:bg-green-100 transition active:scale-98"
              >
      ...
      {/* FAB — single green button that opens modal */}
      <div className="fixed bottom-6 right-4 z-50">
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
