...
${hasCatalog
  ? `- ONLY recommend tests from ${lab.name}'s catalog listed below. Do not suggest tests outside this catalog.
- Match the patient's symptoms to the closest available tests.`
  : `- No catalog is available for this lab. Politely tell the patient to contact the lab directly about available tests.`}
...
${hasCatalog ? `AVAILABLE TESTS:\n${catalogText}` : "No catalog loaded for this lab."}
...
CRITICAL: In SUGGESTED_TESTS, write ONLY the exact test names from the catalog above — no category labels, no brackets, no extra text.
