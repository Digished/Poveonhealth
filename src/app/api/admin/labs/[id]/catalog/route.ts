...
export async function POST(
  ...
  const test = await prisma.labOfferedTest.upsert({
    ...
    create: { lab_id: id, raw_name, category_label: category_label ?? null, synonyms, lab_price, is_active },
    update: { category_label: category_label ?? null, synonyms, lab_price, is_active },
  });

  return NextResponse.json({
    success: true,
    test: { ...test, lab_price: Number(test.lab_price), synonyms: Array.isArray(test.synonyms) ? test.synonyms : [] },
  });
}
