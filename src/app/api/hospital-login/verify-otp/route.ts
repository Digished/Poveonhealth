...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hospital || !hospital.is_active) {
      return NextResponse.json({ error: "No hospital account found for this email." }, { status: 404 });
    }

    const codeHash = createHash("sha256").update(trimmedCode).digest("hex");
    const otp = await prisma.hospitalOtp.findFirst({
    ...
    await prisma.hospitalOtp.update({ where: { id: otp.id }, data: { used: true } });
    ...
    const should_create_pin = !hospital.pin_hash;

    const res = NextResponse.json({ success: true, should_create_pin, hospital_name: hospital.name });
    ...
    console.error("[hospital-login/verify-otp]", err);
