...
function HospitalLoginInner() {
  ...
  async function handleEmailSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!email.trim()) { setError("Please enter your hospital email address."); return; }
    setLoading(true);
    try {
      const res = await fetch("/api/hospital-login/check-pin", {
        method: "POST",
      ...
      if (!res.ok || !data.exists) {
        setError(data.error ?? "No hospital account found for this email. Contact Poveon to get registered.");
        return;
      }
  ...
  async function sendOtp() {
    const res = await fetch("/api/hospital-login/send-otp", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: email.trim() }),
    });
  ...
  async function handlePinSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/hospital-login/verify-pin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleOtpSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/hospital-login/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-dvh bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50 flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-xl font-bold text-slate-800">Hospital Portal</h1>
          <p className="text-sm text-slate-500 mt-1">Receive and manage patient referrals</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-3xl shadow-xl p-6">
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <label htmlFor="hospital-email" className="text-sm font-semibold text-slate-700">Hospital Email Address</label>
                </div>
                <p className="text-xs text-slate-400 mb-3">Enter the email your hospital is registered with on Poveon.</p>
                <input
                  id="hospital-email"
                  type="email"
                  autoComplete="email"
                  placeholder="referrals@hospital.com"
                  value={email}
              ...
              <div className="text-center">
                ...
                {hospitalName && (
                  <p className="text-xs text-slate-500 mb-1 flex items-center justify-center gap-1.5">
                    <Hospital className="w-3.5 h-3.5 text-medical-400" />
                    <span className="font-semibold text-slate-600">{hospitalName}</span>
                  </p>
                )}
                <p className="text-xs text-slate-400 mb-4">Signing in as <span className="font-semibold text-slate-600">{email}</span></p>
          ...
          {stage === "create-pin" && (
            <form onSubmit={handleCreatePin} className="space-y-5">
              ...
              <div className="text-center">
                <button type="button" onClick={() => router.replace("/hospital-dashboard")}
