/**
 * Nigerian states (36 + FCT) and their local government areas — powers the
 * "where are you coming from" predictive fields on the self-service intake
 * form. `fuzzyFilter` gives forgiving matching (prefix > word-prefix >
 * substring > subsequence) so "surul" or "ph" still find the right entry.
 */
...
export const NIGERIA_STATES: { state: string; lgas: string[] }[] = [
  ...
  { state: "FCT (Abuja)", lgas: ["Abaji", "Abuja Municipal (AMAC)", "Bwari", "Gwagwalada", "Kuje", "Kwali"] },
  ...
  { state: "Kogi", lgas: ["Adavi", "Ajaokuta", "Ankpa", "Bassa", "Dekina", "Ibaji", "Idah", "Igalamela-Odolu", "Ijumu", "Kabba/Bunu", "Kogi (Koton Karfe)", "Lokoja", "Mopa-Muro", "Ofu", "Ogori/Magongo", "Okehi", "Okene", "Olamaboro", "Omala", "Yagba East", "Yagba West"] },
  ...
  { state: "Ogun", lgas: ["Abeokuta North", "Abeokuta South", "Ado-Odo/Ota", "Ewekoro", "Ifo", "Ijebu East", "Ijebu North", "Ijebu North East", "Ijebu Ode", "Ikenne", "Imeko Afon", "Ipokia", "Obafemi Owode", "Odeda", "Odogbolu", "Ogun Waterside", "Remo North", "Sagamu", "Yewa North", "Yewa South"] },
  ...
  { state: "Zamfara", lgas: ["Anka", "Bakura", "Birnin Magaji", "Bukkuyum", "Bungudu", "Gummi", "Gusau", "Kaura Namoda", "Maradun", "Maru", "Shinkafi", "Talata Mafara", "Tsafe", "Zurmi"] },
];

export const STATE_NAMES = NIGERIA_STATES.map((s) => s.state);

export function lgasForState(state: string): string[] {
  return NIGERIA_STATES.find((s) => s.state.toLowerCase() === state.toLowerCase())?.lgas ?? [];
}

/** Score one candidate against a query: bigger is better, ≤0 means no match. */
function fuzzyScore(query: string, candidate: string): number {
  ...
  // Any word in the candidate starts with the query ("mun" → "Calabar Municipal")
  if (c.split(/[\s/()-]+/).some((w) => w.startsWith(q))) return 600 - c.length;
  if (c.includes(q)) return 400 - c.indexOf(q);
  // Subsequence match ("srl" → "Surulere") — every query char in order
  let i = 0;
...
/** Rank `options` against `query`, best match first; empty query returns all. */
export function fuzzyFilter(query: string, options: string[], limit = 12): string[] {
